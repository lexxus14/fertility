...
@section('content')
  ...
  <section class="content">
    <div class="col-md-12">
      <div class="card">
        <div class="card-header">
          <h3 class="card-title"><b>Biopsy Result</b></h3>
          ...
          <table id="example1" class="table table-bordered table-striped">
            ...
            <tbody>
              <?php
                $intctr = 0;
              ?>
            @foreach($biopsyresults as $result)
            <?php 
              $intctr++;
            ?>
            <tr>
              ...
              <td>{{$result->Nurse}}</td>
              <td>{{$result->NormalEmbryo}}</td>
              <td>{{$result->AbnormalEmbryo}}</td>
              <td>{{$result->AbnormalEmbryo}}</td>
              <td>{{$result->NumberOfMale}}</td>
              <td>{{$result->NumberOfFemale}}</td>
              <td>{{$result->Notes}}</td>
              <td>
                <a class="btn btn-primary btn-sm float-right" href="{{route('BiopsyResultView')}}/{{$result->id}}">
                  <i class="fas fa-folder"></i>
                        View
                </a>
                <a class="btn btn-info btn-sm float-right" href="{{route('BiopsyResultEdit')}}/{{$result->id}}">
        ...
        <div class="modal-dialog modal-lg">
          <div class="modal-content">
            <div class="modal-header">
              <h4 class="modal-title">Biopsy Result</h4>
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>

            <form id="quickForm" action="{{route('BiopsyResultStore')}}" method="POST" enctype="multipart/form-data">
              {{ csrf_field() }}
              ...
              <div class="modal-body">
                <div class="row">
                  ...
                  <div class="col-12">
                      ...
                      <div class="form-group">
                        <label>Normal Embryo</label>
                        <input type="text" class="form-control" name="txtNormalEmbryo">
                      </div>
                      <div class="form-group">
                        <label>Abnormal Embryo</label>
                        <input type="text" class="form-control" name="txtAbnormalEmbryo">
                      </div>
                      <div class="form-group">
                        <label>Number of Male</label>
                        <input type="text" class="form-control" name="txtNumberOfMale">
                      </div>
                      <div class="form-group">
                        <label>Number of Female</label>
                        <input type="text" class="form-control" name="txtNumberOfFemale">
                      </div>
                      <div class="form-group">
                        <div class="form-group">
                          <label>Note</label>
    ...
    <div class="modal fade" id="modal-delete">
        <div class="modal-dialog">
          <div class="modal-content">
            ...
            <form method="POST" action="{{route('BiopsyResultDelete')}}">
              {{ csrf_field() }}
              <div class="modal-footer justify-content-between">
                <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
