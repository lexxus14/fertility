...
@section('content')
  ...
  <section class="content">
   <form action="{{route('PatientDoctorsPlanUpdate')}}" method="POST" enctype="multipart/form-data" class="needs-validation add-product-form" novalidate="">
    @foreach($docresults as $docresult)
        {{ csrf_field() }}
      <input type="hidden" name="txtpatientId" value="{{$intPatientId}}">
      <input type="hidden" name="txtPatientDoctorPlanId" value="{{$docresult->id}}">
      <div class="row">
        <div class="col-md-12">
          <div class="card card-primary">
            ...
            <div class="card-body">
              ...
              <div class="row">
                <div class="form-group">
                <div class="col-12">
                  <input type="button" value="Add Doctor's Plan" class="btn btn-success float-right" data-toggle="modal" data-target="#open-modal-medicine-treatment">
                </div>
                </div>
              </div>   
              <div class="row">
                ...
                <!-- /.card-header -->
                    ...
                    <table  class="table table-bordered table-striped">
                      ...
                      $intctr = 0;
                      ?>
                      @foreach($patientdoctorsplansubs as $patientdoctorsplansub)
                      <?php 
                      $intctr++;
                      ?>
                      <tr id="R{{$intctr}}">
                        <td class="row-index text-center">
                        <input type="hidden" class="medid" name="txtDoctorsPlanId[]" value="{{$patientdoctorsplansub->doctorplanId}}">
                          <p>{{$intctr}}</p>
                        </td>
                        <td class="text-center">
                        {{$patientdoctorsplansub->description}}
                        </td>                        
                        <td class="text-center">
                          <textarea class="form-control amount amount{{$patientdoctorsplansub->id}}" name="txtnote[]">{{$patientdoctorsplansub->notes}}</textarea>
                        </td>
                        <td class="text-center">
        ...
        <div class="modal-dialog modal-lg">
          <div class="modal-content">
            <div class="modal-header">
              <h4 class="modal-title">Doctor's Plan</h4>
                ...
                <div class="col-12">
                  ...
                  <tbody>
                    <?php
                      $intctr =0;
                    ?>
                    @foreach($doctorsPlans as $doctorsPlan)
                    <?php
                    $intctr++; 
                    ?>
                    <tr>
                      <td>{{$intctr}}</td>
                      <td>{{$doctorsPlan->description}}</td>

                      <td><button type="button" class="btn btn-success add-medicine-treatment" value="{{$doctorsPlan->id}}">Add</button> </td>
                    </tr>
                    @endforeach
                  </tbody>
                </table>                                 
...
<script>
  $(function () {
    ...
    $('.add-medicine-treatment').click(function(){
      ...
      var med_id = $(this).val();
      url = '{{route('DoctorsPlanInfo')}}';

      $.get(url + '/' + med_id, function (data) {
        console.log(data);
            $('#tbody').append(`<tr id="R${++rowIdx}">
              <td class="row-index text-center">
              <input type="hidden" class="medid" name="txtDoctorsPlanId[]" value="${data.id}">
                <p>${rowIdx}</p>
              </td>
              <td class="text-center">
              ${data.description}
