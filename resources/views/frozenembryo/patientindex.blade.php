...
@section('content')
  ...
  <section class="content">
    <div class="col-md-12">
      <div class="card">
        <div class="card-header">
          <h3 class="card-title"><b>Frozen Embryo</b></h3>
          ...
          <table id="example1" class="table table-bordered table-striped">
            ...
            <tbody>
              <?php
                $intctr = 0;
              ?>
            @foreach($frozenembryos as $result)
            <?php 
              $intctr++;
            ?>
            <tr>
              ...
              <td>{{$result->Nurse}}</td>
              <td>{{$result->FrozenEmbryo}}</td>
              <td>{{$result->Notes}}</td>
              <td>
                <a class="btn btn-primary btn-sm float-right" href="{{route('FrozenEmbryoView')}}/{{$result->id}}">
                  <i class="fas fa-folder"></i>
                        View
                </a>
                <a class="btn btn-info btn-sm float-right" href="{{route('FrozenEmbryoEdit')}}/{{$result->id}}">
        ...
        <div class="modal-dialog modal-lg">
          <div class="modal-content">
            <div class="modal-header">
              <h4 class="modal-title">Frozen Embryo</h4>
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>

            <form id="quickForm" action="{{route('FrozenEmbryoStore')}}" method="POST" enctype="multipart/form-data">
              {{ csrf_field() }}
    ...
    <div class="modal fade" id="modal-delete">
        <div class="modal-dialog">
          <div class="modal-content">
            ...
            <form method="POST" action="{{route('FrozenEmbryoDelete')}}">
              {{ csrf_field() }}
              <div class="modal-footer justify-content-between">
                <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
