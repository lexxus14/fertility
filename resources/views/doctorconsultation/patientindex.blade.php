...
@section('content')
  ...
  <div class="content-wrapper">
    		...
    		<div class="row">
	          ...
	          <div class="col-md-12">
	            <div class="card">
                  ...
                  <a href="{{route('DoctorConsultationCreateFemale')}}/{{$intPatientId}}" class="btn btn-success float-right" style="margin-right: 5px;">
                    <i class="fas fa-pencil-alt"></i> New
                  </a>
	              </div>
	              <!-- /.card-header -->
	                  ...
	                  <tbody>
	                    <?php
		                  ...
		                  <tr>
		                    ...
		                    <td>{{$result->Notes}}</td>
		                    <td>
		                      <a class="btn btn-primary btn-sm float-right" href="{{route('DoctorConsultationShowFemale')}}/{{$result->id}}">
		                        <i class="fas fa-folder"></i>
		                              View
		                      </a>

		                      <a class="btn btn-info btn-sm float-right" href="{{route('DoctorConsultationEditFemale')}}/{{$result->id}}">
    ...
    <section class="content"> 
            ...
            <div class="col-md-12">
              <div class="card">
                <div class="card-header">
                  ...
                  <a href="{{route('DoctorConsultationCreateMale')}}/{{$intPatientId}}" class="btn btn-success float-right" style="margin-right: 5px;">
                    <i class="fas fa-pencil-alt"></i> New
                  </a>
                </div>
                <!-- /.card-header -->
                  ...
                  <table id="example1" class="table table-bordered table-striped">
                    ...
                    <tbody>
                      <?php
                          $intctr = 0;
                        ?>
                      @foreach($docresultmale as $result)<!-- lab foreach -->
                      <?php 
                        $intctr++;
                      ?>
                      <tr>
                        ...
                        <td>{{$result->Notes}}</td>
                        <td>
                          <a class="btn btn-primary btn-sm float-right" href="{{route('DoctorConsultationShowMale')}}/{{$result->id}}">
                            <i class="fas fa-folder"></i>
                                  View
                          </a>

                          <a class="btn btn-info btn-sm float-right" href="{{route('DoctorConsultationEditMale')}}/{{$result->id}}">
                            <i class="fas fa-pencil-alt"></i> Edit
                          </a>                       

                          <button type="button" class="btn btn-danger btn-sm open-modal-delete-male float-right" data-toggle="modal" data-target="#modal-delete-male" value="{{$result->id}}"> <i class="fas fa-trash">
                                </i>Delete
                          </button>
                        </td>
                      </tr> 
      ...
      <div class="modal fade" id="modal-delete">
        <div class="modal-dialog">
          <div class="modal-content">
            ...
            <form method="POST" action="{{route('DoctorConsultationDeleteFemale')}}">
              {{ csrf_field() }}
      ...
      <div class="modal fade" id="modal-delete-male">
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
            ...
            <form method="POST" action="{{route('DoctorConsultationDeleteMale')}}">
              {{ csrf_field() }}
              <div class="modal-footer justify-content-between">
                <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                <button type="submit" class="btn btn-primary">Delete</button>
              </div>
              <input type="hidden" id="del_id_male" name="del_id_male" value="0">
              <input type="hidden" name="txtpatientId" value="{{$intPatientId}}">
            </form>
          </div>
          <!-- /.modal-content -->
...
<script src="{{ asset('plugins/datatables-buttons/js/buttons.colVis.min.js')}}"></script>
  ...
  $(document).ready(function(){
    ...
    $('.open-modal-delete-male').click(function(data){
      var id = $(this).val();
      $('#del_id_male').val(id);
    });
