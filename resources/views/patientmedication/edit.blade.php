...
@section('content')
  ...
  <section class="content">
   <form action="{{route('PatientMedicationUpdate')}}" method="POST" enctype="multipart/form-data" class="needs-validation add-product-form" novalidate="">
              ...
              <div class="row">
                ...
                <!-- /.card-header -->
                    ...
                    <table  class="table table-bordered table-striped">
                      ...
                      $intctr = 0;
                      ?>
                      @foreach($patientmedicationsubs as $patientmedicationsub)
                      <?php 
                      $intctr++;
                      ?>
                      <tr id="R{{$intctr}}">
                        <td class="row-index text-center">                        
                        <input type="hidden" class="medid" name="txtmedId[]" value="{{$patientmedicationsub->medicineid}}">
                          <p>{{$intctr}}</p>
                        </td>
      ...
      $.get(url + '/' + med_id, function (data) {
        console.log(data);
            $('#tbody').append(`<tr id="R${++rowIdx}">
              ...
              <input type="hidden" class="medid" name="txtmedId[]" value="${data.id}">
                <p>${rowIdx}</p>
              </td>
              <td class="text-center">
              ${data.description}
